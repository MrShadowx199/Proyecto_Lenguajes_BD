...
-- SYS REFCURSOR DEPARTAMENTOS
CREATE OR REPLACE PROCEDURE GET_DEPARTAMENTO_CURSOR (P_CURSOR OUT SYS_REFCURSOR)
IS
BEGIN
OPEN P_CURSOR FOR
...
-- CURSOR CITAS
SET SERVEROUTPUT ON;
CREATE OR REPLACE PROCEDURE DISPLAY_CITAS IS
    V_SQL VARCHAR2(1000);
    V_ID_PACIENTE CITAS.ID_PACIENTE%TYPE;
    V_ID_DOCTOR CITAS.ID_DOCTOR%TYPE;
    V_FECHA_CITA CITAS.FECHA_CITA%TYPE;
...
BEGIN
    V_SQL := 'SELECT ID_PACIENTE, ID_DOCTOR, FECHA_CITA, HORA_CITA FROM CITAS';

OPEN V_CURSOR FOR V_SQL;

LOOP
FETCH V_CURSOR INTO V_ID_PACIENTE, V_ID_DOCTOR, V_FECHA_CITA, V_HORA_CITA;
